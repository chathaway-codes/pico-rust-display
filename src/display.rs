...
fn lcd_2in_set_window<T: WriteOnlyDataCommand>(
    dis: &mut T,
    x_start: u16,
    y_start: u16,
    x_end: u16,
...
fn lcd_2in_write_command<T: WriteOnlyDataCommand>(
    dis: &mut T,
    com: u8,
) -> Result<(), DisplayError> {
...
fn lcd_2in_write_data_byte<T: WriteOnlyDataCommand>(
    dis: &mut T,
    com: u8,
) -> Result<(), DisplayError> {
    ...
    CS: OutputPin,
{
        ...
        RST: OutputPin,
    {
        ...
        Ok(LCD2InchDisplay { display: display })
    }
}
...
impl<T> Dimensions for LCD2InchDisplay<T> {
    fn bounding_box(&self) -> Rectangle {
        Rectangle {
            ...
            size: Size {
                width: LCD_2IN_WIDTH as u32,
                height: LCD_2IN_HEIGHT as u32,
            },
        }
    }
}
...
impl<T: WriteOnlyDataCommand> DrawTarget for LCD2InchDisplay<T> {
        ...
        I: IntoIterator<Item = Pixel<Self::Color>>,
    {
        for pixel in pixels.into_iter() {
            ...
            self.display
                .send_data(DataFormat::U16BE(&mut [pixel.1.into_storage()]))
                .map_err(|_| DisplayError::Unknown)?;
        }
        Ok(())
    }
    ...
    fn fill_solid(&mut self, area: &Rectangle, color: Self::Color) -> Result<(), Self::Error> {
        let mut buf = [color.into_storage(); LCD_2IN_WIDTH * LCD_2IN_HEIGHT];
        ...
        lcd_2in_set_window(&mut self.display, x, y, x + width, y + height)?;
        self.display
            .send_data(DataFormat::U16BE(&mut buf[0..(width * height) as usize]))
            .map_err(|_| DisplayError::Unknown)?;
        Ok(())
    }
}
